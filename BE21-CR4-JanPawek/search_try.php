<?php
    ...
    $sql = "SELECT * FROM `inventory` WHERE 
                ...
                `author_last_name` LIKE '%$search%' OR 
                `ISBN` LIKE '%$search%' OR 
                `short_des` LIKE '%$search%' OR 
                `long_des` LIKE '%$search%' OR 
                `type` LIKE '%$search%'  OR 
                `publisher_name` LIKE '%$search%' OR 
                `publisher_address` LIKE '%$search%' OR 
                `status_del` LIKE '%$search%' OR 
                `publish_date` LIKE '%$search%'";
    ...
    $sql = "SELECT DISTINCT `title`, `author_first_name`, `author_last_name`, `ISBN`, `short_des`, `long_des`, `type`, `publisher_name`, `publisher_address`, `publish_date`, `status_del` FROM `inventory` WHERE 
                `title` LIKE '%$searchQuery%' OR 
                `author_first_name` LIKE '%$searchQuery%' OR 
                `author_last_name` LIKE '%$searchQuery%' OR 
                `ISBN` LIKE '%$searchQuery%' OR 
                `short_des` LIKE '%$searchQuery%' OR 
                `long_des` LIKE '%$searchQuery%' OR 
                `type` LIKE '%$searchQuery%'  OR 
                `publisher_name` LIKE '%$searchQuery%' OR 
                `publisher_address` LIKE '%$searchQuery%' OR 
                `status_del` LIKE '%$searchQuery%' OR 
                `publish_date` LIKE '%$searchQuery%'";
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarScroll">
      ...
      <form class="d-flex position-relative" role="search" action="" method="GET">
        <input class="form-control me-2" type="text" name="search" id="searchInput" placeholder="Search..." aria-label="Search">
...
<br><br>
    ...
    searchInput.addEventListener('input', function() {
        ...
        fetch(`search.php?query=${encodeURIComponent(searchQuery)}`)
            ...
            .then(data => {
                const suggestions = data.map(item => `<div class="search-suggestion-item">${item}</div>`).join('');
